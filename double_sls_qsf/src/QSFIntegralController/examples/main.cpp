...
#include "QSFIntegralController_terminate.h"
...
// Return Type  : void
//
void main_QSFIntegralController()
{
  double dv[15];
  double dv1[13];
  double ref_1_tmp[5];
  double dv2[4];
  double F[3];
  // Initialize function 'QSFIntegralController' input arguments.
  ...
  argInit_1x5_real_T(ref_1_tmp);
  // Initialize function input argument 'ref_2'.
  // Initialize function input argument 'ref_3'.
  // Call the entry-point 'QSFIntegralController'.
  argInit_15x1_real_T(dv);
  argInit_1x13_real_T(dv1);
  argInit_1x4_real_T(dv2);
  QSFIntegralController(dv, dv1, dv2, ref_1_tmp, ref_1_tmp, ref_1_tmp, F);
}
